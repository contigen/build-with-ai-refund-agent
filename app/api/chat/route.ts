...
Never output plain text formatting when a tool call is more appropriate. Keep conversation minimal and to the point.`,
    tools: {
      selectProductToRefund: {
        description: 'Show a list of recent products eligible for refund for the user to select from.',
        ...
        execute: async () => {
          return {
            products: [
              { id: 'p1', name: 'Echo Dot (5th Gen)', price: 0.50 },
              { id: 'p2', name: 'Sony WH-1000XM5 Headphones', price: 1.00 },
              { id: 'p3', name: 'MacBook Pro 14" (Silver)', price: 5.00 },
            ]
          };
        },
      },
      processRefund: {
        description: 'Process a refund for a selected product and show confirmation.',
        inputSchema: z.object({
          ...
          walletAddress: z.string().describe('The wallet address of the user, we are using arc testnet to do payment'),
        }),
        execute: async ({ productId, productName, amount, walletAddress }) => {
          const privateKey = process.env.AGENT_PRIVATE_KEY;
          const agent = new Wallet(privateKey!, provider);

          const recipientAddress = walletAddress; // Target address
          const amountToSend = amount.toString(); // Sending 1.0 USDC
